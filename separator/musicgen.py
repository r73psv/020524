    ...
    with gr.Blocks() as interface:
        ...
        submit.click(toggle_diffusion, decoder, [diffusion_output, audio_diffusion], queue=False,
                     show_progress=False).then(predict_full,
                                               inputs=[model, model_path, decoder, text, melody, duration, topk, topp,
...
def ui_batched(launch_kwargs):
    with gr.Blocks() as demo:
        ...
        Модель сгенерирует 15 секунд аудио на основе предоставленного вами описания.
        Модель была обучена с использованием описания из стандартного музыкального каталога, описания, которые подойдут лучше всего.
        должен включать некоторую информацию о присутствующих инструментах, а также некоторые предполагаемые варианты использования.
        (например, добавление «идеально подходит для рекламы» может как-то помочь).

        При желании вы можете предоставить эталонный звук, из которого будет извлечена широкая мелодия.
        Затем модель попытается следовать как описанию, так и предоставленной мелодии.
        Для достижения наилучших результатов длина мелодии должна составлять 30 секунд (я знаю, что предоставленные нами образцы 
        не такие...)

        Вы можете получить доступ к большему контролю (более длинное поколение, больше моделей и т. д.), нажав
        <a href="https://huggingface.co/spaces/facebook/MusicGen?duulate=true"
                style="display: inline-block;margin-top: .5em;margin-right: .25em;" цель="_blank">
            <img style="margin-bottom: 0em;display: inline;margin-top: -.25em;"
                src="https://bit.ly/3gLdBN6" alt="Дублировать пространство"></a>
        (тогда вам понадобится платный графический процессор от HuggingFace).
        Если у вас есть графический процессор, вы можете запустить демонстрационную версию градиента 
        локально (для получения дополнительной информации щелкните ссылку на наш репозиторий ниже).
        Наконец, вы можете бесплатно получить графический процессор от Google.
        и запустите демо в [Google Colab.](https://ai.honu.io/red/musicgen-colab).

        См. [github.com/facebookresearch/audiocraft](https://github.com/facebookresearch/audiocraft/blob/main/docs/MUSICGEN.md).
        Больше подробностей. Все семплы генерируются с использованием модели «стереомелодия».
        """)
